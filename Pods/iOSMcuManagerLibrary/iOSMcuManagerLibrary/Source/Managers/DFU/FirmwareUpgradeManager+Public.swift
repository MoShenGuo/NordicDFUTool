...
public extension FirmwareUpgradeManager {
    ...
    /// Start the firmware upgrade. 开始固件升级
    ///
    /// This is the full-featured API to start DFU update, including support for Multi-Image uploads.这是用于启动 DFU 更新的完整功能 API，支持多图像上传。
    /// - parameter images: An Array of (`ImageManager.Image`) to upload. 上传的 `ImageManager.Image` 数组。
    /// - parameter configuration: Fine-tuning of details regarding the upgrade process.升级过程细节的精细设置。
